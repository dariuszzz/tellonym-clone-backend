...
impl JWTUtil {
    ...
    #[must_use]
    pub fn sign_access_jwt(id: i32) -> String {
    ...
    #[must_use]
    pub fn verify_access_jwt(jwt: &str) -> Option<i32> {
        let key: Hmac<Sha256> = Hmac::new_from_slice(dotenv!("ACCESS_SECRET").as_bytes()).unwrap();
        
        let claims: BTreeMap<String, String> = jwt.verify_with_key(&key).unwrap_or_else(|_| None)?;
        
        let username = claims.get("sub")?.clone().parse::<i32>().ok()?;
    ...
    #[must_use]
    pub fn sign_refresh_jwt(id: i32) -> String {
    ...
    #[must_use]
    pub fn verify_refresh_jwt(jwt: &str) -> Option<i32> {
        let key: Hmac<Sha256> = Hmac::new_from_slice(dotenv!("REFRESH_SECRET").as_bytes()).unwrap();
